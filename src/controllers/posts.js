...
const addPost = async (req, res) => {
  ...
  const author = await User.findOne({ email: req.body.author_email }).select(
    "-password"
  );
  const newPost = {
    ...rest,
...
const approvePost = async (req, res) => {
  const admin = await Admin.findOne({ email: "admin@mail.com" });
  const newPostData = admin.requests_posts.filter(
    (e) => e._id.toString() === req.params.postId
  );
  const newPost = new Post(...newPostData);
  const requestsPosts = admin.requests_posts.filter(
    (e) => e._id.toString() !== req.params.postId
  );
  try {
    await Admin.updateOne(
